...
- **Tự động tính trạng thái (Logic Client-side)**: Dựa trên "Ngày Hết Hạn" so với thời gian hiện tại của hệ thống:
  - <= 15 ngày: `Sắp hết hạn` (Cảnh báo màu Vàng/Cam - Amber).
  - < 0 ngày: `Đã hết hạn` (Cảnh báo màu Đỏ).
  - Còn lại: `Đang hoạt động` (Màu Xanh lá).
- **Phân Trang**: Triển khai hệ thống phân trang chuẩn hóa (Pagination) cho danh sách hosting.
...
- **[2026-03-29]** Tích hợp Floating Action Menu (`.row-action-menu`) (Xem chi tiết, Chỉnh sửa, Xóa) mở thông minh không bị tràn viền.
- **[2026-03-29]** Hoàn thiện **Modal Chi Tiết Hosting** với giao diện header gradient, card thông tin dạng Grid, tích hợp nút "Chỉnh Sửa/Xóa" bám sát thao tác.
- **[2026-03-29]** Nâng cấp UX bằng **Custom Delete Modal** (Giao diện "Xác nhận xóa hosting").
- **[2026-03-29]** Xây dựng module **Toast Action**: Trượt từ Bottom-Right, hiển thị thông báo Loading Spinner và tự động chuyển sang trạng thái Success (Checkmark) áp dụng nhất quán cho Thêm, Cập nhật, và Xóa.
- **[2026-03-29]** Tích hợp tính năng **Bulk Action Bar** và API CRUD cơ bản.
- **[2026-03-29]** Chuẩn hóa giao diện **Modal Header**: Thiết kế Brand-boxed icon mới cho module Hosting.
- **[2026-03-29]** Đồng bộ hệ thống **Dropdown Select**: Sử dụng giao diện premium đồng bộ toàn Dashboard.
